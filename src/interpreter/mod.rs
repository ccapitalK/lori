...
impl Interpreter {
    ...
    fn set_local(&mut self, var_name: &str, value: LuaValue) {
        self.scopes.last_mut().unwrap().insert(var_name.to_string(), value);
    }
...
impl ASTVisitor<InterpreterResult> for Interpreter {
    ...
    fn visit_simple_exp(&mut self, se: &mut SimpleExp)  -> Option<InterpreterResult> {
        Some(InterpreterResult::LuaValue(match se {
            ...
            &mut SimpleExp::TableConstructor(ref mut tc) => return self.visit_table_constructor(tc), 
        }))
    }
        ...
        match exp {
            ...
            &mut Exp::UnaryOp(UnOp::Len, ref mut e) => {
                match try_ir!(self.visit_exp(e)).unwrap().get_lua_value() {
                    LuaValue::String(se) => Some(InterpreterResult::LuaValue(
                        LuaValue::Number(se.len() as f64)
                    )),
            ...
            &mut Exp::BinaryOp(ref mut e1, op, ref mut e2) => {
                ...
                let rv = match op {
                    ...
                    BinOp::Pow          => LuaValue::Number(
                        lv1.to_number().powf(lv2.to_number())
                    ),
                    BinOp::Concat       => LuaValue::String(
                        format!("{}{}", lv1.to_string(), lv2.to_string())
                    ),
                    ...
                    BinOp::Equals       => LuaValue::Boolean(
                        lv1 == lv2
                    ),
                    BinOp::NotEquals    => LuaValue::Boolean(
                        lv1 == lv2
                    ),
                    BinOp::And          => LuaValue::Boolean(
                        lv1.to_bool() && lv2.to_bool()
                    ),
                    BinOp::Or           => LuaValue::Boolean(
                        lv1.to_bool() || lv2.to_bool()
                    ),
            ...
            &mut Stat::LocalAssign(ref name_list, ref mut exp_list) => {
                ...
                while index < name_list.len() || index < exp_list.len() {
                    let exp = if index < exp_list.len() {
                        try_ir!(self.visit_exp(&mut exp_list[index])).unwrap().get_lua_value()
                    } else {
                        LuaValue::Nil
                    };
...
mod tests {
    ...
    fn test_eval_exp() {
        ...
        assert_eq!(interpreter.get_variable("e"), LuaValue::String("Hello".to_string()));
        ...
        assert_eq!(interpreter.get_variable("p"), LuaValue::String("hi".to_string()));
